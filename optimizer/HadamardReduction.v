...
Definition apply_H_equivalence1 {dim} q (l : opt_ucom_l dim) := 
  replace_single_qubit_pattern l q 
    (UO_H  :: UO_P :: UO_H :: []) 
    (UO_PDAG :: UO_H :: UO_PDAG :: [])
    match_gate.

Definition apply_H_equivalence2 {dim} q (l : opt_ucom_l dim) := 
  replace_single_qubit_pattern l q 
    (UO_H :: UO_PDAG :: UO_H :: []) 
    (UO_P :: UO_H :: UO_P :: [])
    match_gate.

Definition apply_H_equivalence3 {dim} q (l : opt_ucom_l dim) := 
  match (next_single_qubit_gate l q) with
  | Some (l1, UO_H, l2) =>
      let l := l1 ++ l2 in
      match (next_two_qubit_gate l q) with
      | Some (l1, UO_CNOT, m, n, l2) => 
          if (q =? m) 
...
Definition apply_H_equivalence4 {dim} q (l : opt_ucom_l dim) :=
  match (remove_single_qubit_pattern l q (UO_H :: UO_P :: []) match_gate) with
  | None => None
  | Some l1 =>
      match (next_two_qubit_gate l1 q) with
      | Some (l2, UO_CNOT, q1, q2, l3) =>
          if q =? q2 
          then match (remove_single_qubit_pattern l3 q (UO_PDAG :: UO_H :: []) match_gate) with
               | None => None
               | Some l4 =>
...
Definition apply_H_equivalence5 {dim} q (l : opt_ucom_l dim) :=
  match (remove_single_qubit_pattern l q (UO_H :: UO_PDAG :: []) match_gate) with
  | Some l1 =>
      match (next_two_qubit_gate l1 q) with
      | Some (l2, UO_CNOT, q1, q2, l3) =>
          if q =? q2 
          then match (remove_single_qubit_pattern l3 q (UO_P :: UO_H :: []) match_gate) with
               | Some l4 =>
...
Definition apply_H_equivalence {dim} (l : opt_ucom_l dim) (q : nat) : option (opt_ucom_l dim) :=
...
Fixpoint apply_H_equivalences {dim} (l : opt_ucom_l dim) (n: nat) : opt_ucom_l dim :=
  match n with
  | 0 => l
  | S n' => 
      match l with
...
Definition hadamard_reduction {dim} (l : opt_ucom_l dim) := 
...
Definition hadamard_reduction_test1 : opt_ucom_l 4 :=
...
Definition hadamard_reduction_test2 : opt_ucom_l 4 :=
...
Lemma apply_H_equivalence1_sound : forall {dim} (l l' : opt_ucom_l dim) q,
...
Lemma apply_H_equivalence2_sound : forall {dim} (l l' : opt_ucom_l dim) q,
...
Lemma apply_H_equivalence3_sound : forall {dim} (l l' : opt_ucom_l dim) q,
...
Proof.
  ...
  - destruct (last_single_qubit_gate g2 n) eqn:lsqg; try discriminate.
    ...
    repeat (try rewrite opt_to_base_ucom_l_app;
    ...
    apply uc_app_congruence; try reflexivity.
    rewrite <- app_assoc.
    assert ([@App1 _ dim UO_H n0] ++ [App1 UO_H n] =l= [App1 UO_H n] ++ [App1 UO_H n0]).
...
Lemma apply_H_equivalence4_sound : forall {dim} (l l' : opt_ucom_l dim) q,
  apply_H_equivalence4 q l = Some l' ->
  l =l= l'.
Proof.
  intros. 
  unfold apply_H_equivalence4 in H.
  destruct (remove_single_qubit_pattern l q (UO_H :: UO_P :: [])) eqn:rsqp1; try discriminate.
  ...
  destruct (remove_single_qubit_pattern g0 q (UO_PDAG :: UO_H :: [])) eqn:rsqp2; try discriminate.
  apply remove_single_qubit_pattern_correct in rsqp2.
  inversion H; subst.
  simpl in *.
  rewrite rsqp1, rsqp2.
  repeat rewrite app_comm_cons.
  rewrite (cons_to_app (App1 UO_H n)).
  rewrite (cons_to_app (App1 UO_P n)).
  rewrite (does_not_reference_commutes_app1 _ UO_P _ dnr). 
  rewrite app_assoc.
  rewrite (does_not_reference_commutes_app1 _ UO_H _ dnr).
  clear.
  ...
  - do 3 (apply f_equal2; trivial); solve_matrix;
      rewrite Cexp_neg, Cexp_PI2; repeat group_radicals; lca.
  - do 5 (apply f_equal2; trivial); solve_matrix;
      rewrite Cexp_neg, Cexp_PI2; repeat group_radicals; lca.
Qed.

Lemma apply_H_equivalence5_sound : forall {dim} (l l' : opt_ucom_l dim) q,
  apply_H_equivalence5 q l = Some l' ->
  l =l= l'.
Proof.
  intros. 
  unfold apply_H_equivalence5 in H.
  destruct (remove_single_qubit_pattern l q (UO_H :: UO_PDAG :: [])) eqn:rsqp1; try easy.
  apply remove_single_qubit_pattern_correct in rsqp1.
  destruct (next_two_qubit_gate g q) eqn:ntqg; try discriminate.
  repeat destruct p; dependent destruction o.
  specialize (ntqg_l1_does_not_reference _ _ _ _ _ _ _ ntqg) as dnr.
  apply ntqg_preserves_structure in ntqg; subst.
  bdestruct (q =? n); try discriminate.
  destruct (remove_single_qubit_pattern g0 q (UO_P :: UO_H :: [])) eqn:rsqp2; try discriminate.
  apply remove_single_qubit_pattern_correct in rsqp2.
  inversion H; subst.
  simpl in *.
  rewrite rsqp1, rsqp2.
  repeat rewrite app_comm_cons.
  rewrite (cons_to_app (App1 UO_H n)).
  rewrite (cons_to_app (App1 UO_PDAG n)).
  rewrite (does_not_reference_commutes_app1 _ UO_PDAG _ dnr). 
  rewrite app_assoc.
  rewrite (does_not_reference_commutes_app1 _ UO_H _ dnr).
  clear.
  repeat rewrite <- app_assoc; simpl.
...
Lemma apply_H_equivalence_sound : forall {dim} (l l' : opt_ucom_l dim) q,
...
Lemma apply_H_equivalences_sound: forall {dim} (l : opt_ucom_l dim) n, 
...
Proof. 
  ...
  - dependent destruction o.
    destruct (apply_H_equivalence (App1 UO_H n0 :: l) n0) eqn:res.
...
Lemma hadamard_reduction_sound: forall {dim} (l : opt_ucom_l dim), 
  hadamard_reduction l ≅l≅ l.
Proof. intros. symmetry. apply apply_H_equivalences_sound. Qed.

Lemma hadamard_reduction_WT: forall {dim} (l : opt_ucom_l dim),
