...
Definition Rz_commute_rule1 {dim} q (l : opt_ucom_l dim) :=
  match (next_single_qubit_gate l q) with
  | Some (l1, UO_H, l2) => 
      match (next_two_qubit_gate l2 q) with
      | Some (l3, UO_CNOT, q1, q2, l4) =>
          if q =? q2
          then match (next_single_qubit_gate l4 q) with
               | Some (l5, UO_H, l6) => Some (l1 ++ [H q] ++ l3 ++ [CNOT q1 q] ++ l5 ++ [H q], l6) 
               | _ => None
               end
          else None
      | _ => None
...
Definition Rz_commute_rule2 {dim} q (l : opt_ucom_l dim) :=
  match (next_two_qubit_gate l q) with
  | Some (l1, UO_CNOT, q1, q2, l2) => 
      if q =? q2
      then match (next_single_qubit_gate l2 q) with
           | Some (l3, UO_Rzπ k as u, l4) =>
               match (next_two_qubit_gate l4 q) with
               | Some (l5, UO_CNOT, q3, q4, l6) => 
...
Definition Rz_commute_rule3 {dim} q (l : opt_ucom_l dim) :=
  match (next_two_qubit_gate l q) with
  | Some (l1, UO_CNOT, q1, q2, l2) => 
      if q =? q1
...
Definition Rz_cancel_rule {dim} q k (l : opt_ucom_l dim) :=
  ...
  | Some (l1, UO_Rzπ k', l2) => 
      ...
      if (k'' <? 8)%Z then Some (App1 (UO_Rzπ k'') q :: (l1 ++ l2))
      else Some (App1 (UO_Rzπ (k'' - 8)%Z) q :: (l1 ++ l2)) 
  | _ => None
  end.
 *)
...
Definition H_cancel_rule {dim} q (l : opt_ucom_l dim)  :=
  match next_single_qubit_gate l q with
  | Some (l1, UO_H, l2) => Some (l1 ++ l2)
  | _ => None
  end.

(** Propagation rules for X **)

Definition X_commute_rule {dim} q (l : opt_ucom_l dim) :=
  match (next_two_qubit_gate l q) with
  | Some (l1, UO_CNOT, q1, q2, l2) => 
      if q =? q2
      then Some (l1 ++ [CNOT q1 q2], l2)
...
Definition X_cancel_rule {dim} q (l : opt_ucom_l dim) :=
  match (next_single_qubit_gate l q) with
  | Some (l1, UO_X, l2) => Some (l1 ++ l2)
  | _ => None
  end.

(** Propagation rules for CNOT **)

Definition CNOT_commute_rule1 {dim} q1 (l : opt_ucom_l dim) : option (opt_ucom_l dim * opt_ucom_l dim) :=
  match (next_single_qubit_gate l q1) with
  | Some (l1, UO_Rzπ k as u, l2) => Some ([App1 u q1], l1 ++ l2)
  | _ => None
  end.

Definition CNOT_commute_rule2 {dim} q1 q2 (l : opt_ucom_l dim) :=
...
Definition CNOT_commute_rule3 {dim} q1 q2 (l : opt_ucom_l dim) :=
...
Definition CNOT_commute_rule4 {dim} q1 q2 (l : opt_ucom_l dim) :=
  ...
  | Some (l1, UO_H, l2) => 
      ...
      | Some (l3, UO_CNOT, q1', q2', l4) => 
          ...
          then match (next_single_qubit_gate l4 q2) with
               | Some (l5, UO_H, l6) => Some (l1 ++ [H q2] ++ l3 ++ [CNOT q2 q2'] ++ [H q2], l5 ++ l6)
               | _ => None
               end
          else None
      | _ => None
...
Definition CNOT_cancel_rule {dim} q1 q2 (l : opt_ucom_l dim) :=
  match next_two_qubit_gate l q1 with
  | Some (l1, UO_CNOT, q1', q2', l2) => 
...
Definition propagate_Rz {dim} k (l : opt_ucom_l dim) q n :=
  propagate l (Rz_commute_rules q) [Rz_cancel_rule q k] n.

Definition propagate_H {dim} (l : opt_ucom_l dim) q :=
  propagate l [] [H_cancel_rule q] 1%nat.

Definition propagate_X {dim} (l : opt_ucom_l dim) q n :=
  propagate l [X_commute_rule q] [X_cancel_rule q] n.

Definition propagate_CNOT {dim} (l : opt_ucom_l dim) (q1 q2 n : nat) :=
  propagate l (CNOT_commute_rules q1 q2) [CNOT_cancel_rule q1 q2] n.

Fixpoint cancel_single_qubit_gates' {dim} (l : opt_ucom_l dim) (n: nat) : opt_ucom_l dim :=
  match n with
  | 0 => l
  | S n' => match l with
           | App1 (UO_Rzπ k) q :: t => 
               match propagate_Rz k t q (length t) with
               | None => (App1 (UO_Rzπ k) q) :: cancel_single_qubit_gates' t n'
...
Definition cancel_single_qubit_gates {dim} (l : opt_ucom_l dim) := 
  cancel_single_qubit_gates' l (length l).

Fixpoint cancel_two_qubit_gates' {dim} (l : opt_ucom_l dim) (n: nat) : opt_ucom_l dim :=
  match n with
  | 0 => l
  | S n' => match l with
...
Definition cancel_two_qubit_gates {dim} (l : opt_ucom_l dim) := 
...
Lemma Rz_Rz_m8_combine : forall {dim} q k k', 
    @App1 _ dim (UO_Rzπ k) q :: App1 (UO_Rzπ k') q :: [] =l=
    App1 (UO_Rzπ (k+k'-2 * DEN)) q :: [].
Proof.
  intros.
  Local Opaque Z.mul.
  unfold uc_equiv_l; simpl.
...
Proof.
  ...
  replace (IZR (2 * DEN) * PI / IZR DEN)%R with (2 * PI)%R by (unfold DEN; simpl; lra).
  rewrite phase_2pi.
  reflexivity.
Qed.

Lemma Rz_commutes_with_H_CNOT_H : forall {dim} k q1 q2,
  @App1 _ dim (UO_Rzπ k) q2 :: H q2 :: CNOT q1 q2 :: H q2 :: [] =l= H q2 :: CNOT q1 q2 :: H q2 :: App1 (UO_Rzπ k) q2 :: [].
Proof.
  intros.
  unfold uc_equiv_l; simpl.
  rewrite 2 SKIP_id_r.
...
Lemma Rz_commutes_with_CNOT_Rz_CNOT : forall {dim} k k' q1 q2,
  @App1 _ dim (UO_Rzπ k) q2 :: CNOT q1 q2 :: App1 (UO_Rzπ k') q2 :: CNOT q1 q2 :: [] =l= CNOT q1 q2 :: App1 (UO_Rzπ k') q2 :: CNOT q1 q2 :: App1 (UO_Rzπ k) q2 :: [].
Proof.
  intros.
  unfold uc_equiv_l; simpl.
  rewrite 2 SKIP_id_r.
  ...
  - Qsimpl. 
    ...
    replace
    (σx × phase_shift (IZR k' * PI / IZR DEN) × σx × phase_shift (IZR k * PI / IZR DEN))
      with
    (phase_shift (IZR k * PI / IZR DEN) × σx × phase_shift (IZR k' * PI / IZR DEN) × σx)
      by solve_matrix.
    reflexivity.
  - Qsimpl.
    rewrite 2 phase_mul. rewrite Rplus_comm.
...
Lemma Rz_commutes_with_CNOT : forall {dim} k q1 q2,
  @App1 _ dim (UO_Rzπ k) q1 :: CNOT q1 q2 :: [] =l= CNOT q1 q2 :: App1 (UO_Rzπ k) q1 :: [].
Proof.
  intros.
  unfold uc_equiv_l; simpl.
  rewrite 2 SKIP_id_r.
  ...
  autorewrite with eval_db.
  gridify.
  - replace  (∣1⟩⟨1∣ × phase_shift (IZR k * PI / IZR DEN))
      with (phase_shift (IZR k * PI / IZR DEN) × ∣1⟩⟨1∣) by solve_matrix.
    replace  (∣0⟩⟨0∣ × phase_shift (IZR k * PI / IZR DEN))
      with (phase_shift (IZR k * PI / IZR DEN) × ∣0⟩⟨0∣) by solve_matrix.
    reflexivity.
  - replace  (∣1⟩⟨1∣ × phase_shift (IZR k * PI / IZR DEN))
      with (phase_shift (IZR k * PI / IZR DEN) × ∣1⟩⟨1∣) by solve_matrix.
    replace  (∣0⟩⟨0∣ × phase_shift (IZR k * PI / IZR DEN))
...
Lemma H_H_cancel : forall {dim} q, 
  q < dim -> @App1 _ dim UO_H q :: App1 UO_H q :: [] =l= [].
Proof.
  intros.
  unfold uc_equiv_l, uc_equiv; simpl.
...
Lemma X_X_cancel : forall {dim} q, 
  q < dim -> @App1 _ dim UO_X q :: App1 UO_X q :: [] =l= [].
Proof.
  intros.
  unfold uc_equiv_l, uc_equiv; simpl.
...
Lemma CNOT_CNOT_cancel : forall {dim} q1 q2, 
  q1 < dim -> q2 < dim -> q1 <> q2 -> 
  @App2 _ dim UO_CNOT q1 q2 :: App2 UO_CNOT q1 q2 :: [] =l= [].
Proof.
  intros.
  unfold uc_equiv_l, uc_equiv; simpl.
  autorewrite with eval_db; try lia.
...
Lemma propagate_Rz_sound : forall {dim} k (l : opt_ucom_l dim) q n l',
  q < dim ->
  propagate_Rz k l q n = Some l' ->
  App1 (UO_Rzπ k) q :: l =l= l'.
Proof.
  unfold propagate_Rz.
  ...
  - clear l l' res.
    ...
    repeat destruct p; dependent destruction o; try discriminate.
    inversion res; subst.
    rewrite (nsqg_commutes _ _ _ _ _ nsqg).
    rewrite app_comm_cons.
    rewrite Rz_Rz_combine; reflexivity.
  - clear l l' n res Hq.
    ...
    + unfold Rz_commute_rule1 in res.
      ...
      repeat destruct p; dependent destruction o.
      ...
      rewrite (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg0).
      repeat rewrite <- (app_assoc _ _ g2).
      rewrite 2 (does_not_reference_commutes_app1 _ UO_H _ dnrg2).
      rewrite <- (app_assoc g0).
      rewrite (app_assoc _ g2).
      rewrite (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg2).
      repeat rewrite <- (app_assoc _ g4).
      rewrite <- 2 (does_not_reference_commutes_app1 _ UO_H _ dnrg4).
      repeat rewrite <- (app_assoc _ _ [App1 (UO_Rzπ k) n]).
      rewrite <- (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg4).
      repeat rewrite <- app_assoc.  
    ...
    + unfold Rz_commute_rule2 in res.
      ...
      rewrite (cons_to_app (App1 (UO_Rzπ k0) n1) (g4 ++ _)).
      repeat rewrite app_assoc.
      rewrite (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg0).
      repeat rewrite <- (app_assoc _ _ g2).
      rewrite 2 (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnrg2' dnrg2).
      rewrite <- (app_assoc g0).
      rewrite (app_assoc _ g2).
      rewrite (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg2).
      repeat rewrite <- (app_assoc _ g4).
      rewrite <- 2 (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnrg4' dnrg4).
      repeat rewrite <- (app_assoc _ _ [App1 (UO_Rzπ k) n1]).
      rewrite <- (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnrg4).
      repeat rewrite <- app_assoc.  
      do 2 (apply uc_app_congruence; try reflexivity).
      repeat rewrite app_assoc.
    ...
    + unfold Rz_commute_rule3 in res.
      ...
      subst.
      inversion res; subst.
      rewrite (cons_to_app (App1 (UO_Rzπ k) n0) (g0 ++ _)).
      repeat rewrite app_assoc.
      apply uc_app_congruence; try reflexivity. 
      rewrite (does_not_reference_commutes_app1 _ (UO_Rzπ k) _ dnr).
      repeat rewrite <- app_assoc.
      apply uc_app_congruence; try reflexivity. 
      simpl.
...
Lemma propagate_Rz_WT : forall {dim} k (l : opt_ucom_l dim) q n l',
  q < dim ->
  uc_well_typed_l l ->
  propagate_Rz k l q n = Some l' ->
  uc_well_typed_l l'.
...
Lemma propagate_H_sound : forall {dim} (l : opt_ucom_l dim) q l',
  q < dim ->
...
Lemma propagate_H_WT : forall {dim} (l : opt_ucom_l dim) q l',
  q < dim ->
  uc_well_typed_l l -> 
  propagate_H l q = Some l' ->
  uc_well_typed_l l'.
...
Lemma propagate_X_sound : forall {dim} (l : opt_ucom_l dim) q n l',
  q < dim ->
...
Proof.
  ...
  - clear l l' n res Hq.
    ...
    apply ntqg_preserves_structure in ntqg.
    inversion res; subst.
    rewrite (cons_to_app (App1 UO_X n) (g0 ++_)).
    repeat rewrite app_assoc.
    rewrite (does_not_reference_commutes_app1 _ UO_X _ dnr). 
    apply uc_app_congruence; try reflexivity.
    repeat rewrite <- app_assoc.
    apply uc_app_congruence; try reflexivity.
    simpl.
...
Lemma propagate_X_WT : forall {dim} (l : opt_ucom_l dim) q n l',
  q < dim ->
  uc_well_typed_l l ->
  propagate_X l q n = Some l' ->
  uc_well_typed_l l'.
...
Lemma propagate_CNOT_sound : forall {dim} (l : opt_ucom_l dim) q1 q2 n l',
  q1 < dim ->
  q2 < dim -> 
  q1 <> q2 ->
  propagate_CNOT l q1 q2 n = Some l' ->
  App2 UO_CNOT q1 q2 :: l =l= l'.
Proof.
  ...
  - clear l l' res.
    ...
    rewrite app_comm_cons.
    rewrite cons_to_app.
    rewrite (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnr1 dnr2).
    rewrite app_assoc.
    rewrite <- (app_assoc g0).
    simpl.
  ...
  - clear l l' n res.
    ...
    + unfold CNOT_commute_rule2 in res.
      ...
      inversion res; subst.
      rewrite (cons_to_app (App2 UO_CNOT q1 n) (g0 ++ _)).
      repeat rewrite app_assoc.
      rewrite (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnr2 dnr1).
      apply uc_app_congruence; try reflexivity.
      repeat rewrite <- app_assoc.
      apply uc_app_congruence; try reflexivity.
    ...
    + unfold CNOT_commute_rule3 in res.
      ...
      inversion res; subst.
      rewrite (cons_to_app (App2 UO_CNOT n0 q2) (g0 ++ _)).
      repeat rewrite app_assoc.
      rewrite (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnr1 dnr2).
      apply uc_app_congruence; try reflexivity.
      repeat rewrite <- app_assoc.
    ...
    + unfold CNOT_commute_rule4 in res.
      ...
      repeat rewrite app_assoc.
      rewrite (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnrg0' dnrg0).
      repeat rewrite <- (app_assoc _ _ g2).
      rewrite 2 (does_not_reference_commutes_app1 _ UO_H _ dnrg2).
      rewrite <- (app_assoc g0).
      rewrite (app_assoc _ g2).
      rewrite (does_not_reference_commutes_app2 _ UO_CNOT _ _ dnrg2' dnrg2).
      rewrite <- (app_assoc _ g4).
      rewrite <- (does_not_reference_commutes_app1 _ UO_H _ dnrg4).
      repeat rewrite <- app_assoc.  
      do 2 (apply uc_app_congruence; try reflexivity).
      repeat rewrite app_assoc.
      do 2 (apply uc_app_congruence; try reflexivity).
...
Lemma propagate_CNOT_WT : forall {dim} (l : opt_ucom_l dim) q1 q2 n l',
  q1 < dim ->
  q2 < dim -> 
  q1 <> q2 ->
  uc_well_typed_l l ->
...
Lemma cancel_single_qubit_gates_sound : forall {dim} (l : opt_ucom_l dim),
  uc_well_typed_l l -> cancel_single_qubit_gates l =l= l.
Proof.
  intros dim l.
  unfold cancel_single_qubit_gates.
  remember (length l) as n. clear Heqn.
  gen l. induction n; intros l WT; try reflexivity.
  simpl.
  ...
  - dependent destruction o.
    ...
    + destruct (propagate_X l n0 (length l)) eqn:prop.
      ...
      rewrite IHn; try reflexivity; try assumption.
    + destruct (propagate_Rz k l n0 (length l)) eqn:prop.
...
Lemma cancel_single_qubit_gates_WT: forall {dim} (l : opt_ucom_l dim),
...
Lemma cancel_two_qubit_gates_sound : forall {dim} (l : opt_ucom_l dim),
...
Lemma cancel_two_qubit_gates_WT: forall {dim} (l : opt_ucom_l dim),
