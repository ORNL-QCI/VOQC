...
Definition optimize {dim} (l : opt_ucom_l dim) : opt_ucom_l dim :=
...
Definition optimize_check_for_type_errors {dim} (l : opt_ucom_l dim) 
    : option (opt_ucom_l dim) :=
  if opt_list_well_typed_b dim l
...
Lemma optimize_sound : forall {dim} (l : opt_ucom_l dim),
...
Lemma optimize_check_for_type_errors_sound : forall {dim} (l l' : opt_ucom_l dim),
  optimize_check_for_type_errors l = Some l' ->
  l' ≅l≅ l.
Proof.
  intros dim l l' H.
  unfold optimize_check_for_type_errors in H.
  destruct (opt_list_well_typed_b dim l) eqn:WTb; try discriminate.
  inversion H; subst.
